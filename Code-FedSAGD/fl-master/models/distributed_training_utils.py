...
def get_mdl_params_w(W_dict, n_par=None):
    if n_par is None:
        n_par = 0
        for param in W_dict.values():
            n_par += param.numel()  # 直接使用 numel() 计算元素数量

    # 初始化参数矩阵（这里只有一行，因为只处理一个模型）
    param_mat = np.zeros((1, n_par), dtype='float32')

    # 填充参数矩阵
    idx = 0
    for param in W_dict.values():
        temp = param.cpu().numpy().reshape(-1)  # 展平为1维数组
        param_mat[0, idx:idx + len(temp)] = temp
        idx += len(temp)
...
class Client(DistributedTrainingDevice):
    ...
    def synchronize_with_server(self, server):
        ...
        self.W = {name: value for name, value in self.model.named_parameters()}
        if self.args.method == 'fedacg':
            add_mome2(target=self.W, source1=server.W, source2=server.m_buff, beta_1=1, beta_2=server.args.beta_)
        # if self.args.method == 'fedadam':
        #     add_mome2(target=self.W, source1=server.W, source2=server.mt, beta_1=1, beta_2=server.args.beta_)
    ...
    def compute_bias(self, server):
        if self.args.method == 'fedprox':
            ...
            # scale_ts(target=self.bias, source=server.v, scaling= self.args.beta_)
            # self.state_params_diff = torch.tensor(get_other_params([self.bias], self.n_par)[0],
            #                                       dtype=torch.float32,
            #                                       device=self.args.device)
            # cld_mdl_param = torch.tensor(get_mdl_params([self.model], self.n_par)[0], dtype=torch.float32, device=self.args.device)
            # self.state_params_diff = self.args.mu * (-cld_mdl_param)
            cld_mdl_param = torch.tensor(get_mdl_params([self.model], self.n_par)[0], dtype=torch.float32, device=self.args.device)
            self.state_params_diff_prox = self.args.mu * (-cld_mdl_param)
    ...
    def train_cnn(self, server, iter):
        ...
        if (self.args.method == 'fedavg' or 'ours' or self.args.method == 'fedcm'  or self.args.method == 'fedavgm' or self.args.method == 'fedadam'
                or self.args.method == 'fedavg_gnag' or self.args.method == 'fed_nesterov' or self.args.method == 'mime'
                or self.args.method == 'scaffold' or self.args.method == 'fastslowmo' ):
            optimizer = torch.optim.SGD(self.model.parameters(), lr=self.args.lr, momentum=self.args.momentum,
                                       weight_decay=0)

        if self.args.method == 'fedprox' or self.args.method == 'fedacg': #
            optimizer = torch.optim.SGD(self.model.parameters(), lr=self.args.lr, momentum=self.args.momentum,
                                        weight_decay=self.args.mu)
        ...
        if self.args.method == 'feddyn':
            optimizer = torch.optim.SGD(self.model.parameters(), lr=self.args.lr, momentum=self.args.momentum,
                                        weight_decay=self.args.coe)

        scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=1, gamma=1)

        if self.args.method == 'fastslowmo':
            self.yt = copy.deepcopy(server.yl)
        ...
        for iter in range(self.args.local_ep):
            ...
            for i in range(self.local_epoch):
                ...
                if self.args.method == 'ours':
                    loss_prox = torch.sum((self.args.mu / 2) * (local_par_list ** 2)
                                      + local_par_list * self.state_params_diff_prox)

                if (self.args.method == 'fedavg' or self.args.method == 'fedadam' or self.args.method == 'fedcm'
                        or self.args.method == 'fedavgm' or self.args.method == 'fedavg_gnag'
                        or self.args.method == 'fed_localnesterov' or self.args.method =='fed_localnesterov_nocom' or self.args.method == 'fastslowmo'):
                    loss = loss_f_i 
                elif self.args.method == 'fedprox' or self.args.method == 'scaffold' or self.args.method == 'feddyn' or self.args.method == 'fedacg':
                ...
                if self.args.method == 'ours':
                    ...
                    grad = {name: value.grad for name, value in self.model.named_parameters()} #gt
                    scale_ts(target=self.nt, source=grad, scaling=1)
                    if i == 0:
                        add_mome2(target=self.mt, source1=server.v, source2=grad, beta_1=self.args.beta_, beta_2=1)
                        # add_mome2(target=self.mt, source1=server.v, source2=grad, beta_1=0.9, beta_2=0.1)
                        for name, param in self.model.named_parameters():
                            if param.grad is not None:
                                param.grad = self.mt[name] + self.args.lammda * param.data
                        torch.nn.utils.clip_grad_norm_(parameters=self.model.parameters(), max_norm=max_norm)
                        grad = {name: value.grad for name, value in self.model.named_parameters()}
                        scale_ts(target=self.mt, source=grad, scaling=1)
                    else:
                        self.gt = {name: torch.zeros(value.shape).to(self.args.device) for name, value in self.W.items()}
                        for name in self.W:
                            self.gt[name].data =grad[name].data.clone() + self.args.beta_0 * (grad[name].data.clone() - self.vt[name].data.clone())
                        add_mome2(target=self.mt, source1=server.v, source2=self.gt, beta_1=self.args.beta_, beta_2=1)
                        for name, param in self.model.named_parameters():
                            if param.grad is not None:
                                param.grad = self.mt[name] + self.args.lammda * param.data
                        torch.nn.utils.clip_grad_norm_(parameters=self.model.parameters(), max_norm=max_norm)
                        grad = {name: value.grad for name, value in self.model.named_parameters()}
                        scale_ts(target=self.mt, source=grad, scaling=1)
                    scale_ts(target=self.vt, source=self.nt, scaling=1)
                    

                loss.backward()
                torch.nn.utils.clip_grad_norm_(parameters=self.model.parameters(), max_norm=max_norm)

                if self.args.method == 'fedcm':
                    self.gradient = {name: value.grad for name, value in self.model.named_parameters()}
                    # beta1 = self.args.beta_

                    add_mome2(target=self.mt, source1=self.gradient, source2=server.dW, beta_1=self.args.beta_, beta_2=1-self.args.beta_)
                    add_mome2(target=self.W, source1=self.W, source2=self.mt, beta_1=1,
                              beta_2=-self.args.lr)

                elif self.args.method == 'fed_localnesterov' or self.args.method == 'fed_localnesterov_nocom':
                    self.gradient = {name: value.grad for name, value in self.model.named_parameters()}
                    ...
                    gamma = 0.15
                    self.gradient = {name: value.grad for name, value in self.model.named_parameters()}
                    y_old = {name: torch.zeros(value.shape).to(self.args.device) for name, value in self.W.items()}
                    temp = {name: torch.zeros(value.shape).to(self.args.device) for name, value in self.W.items()}
                    scale_ts(target=y_old, source=self.yt, scaling=1)
                    add_mome2(target=self.yt, source1=self.W, source2=self.gradient, beta_1=1, beta_2=-self.args.lr)
                    add_mome2(target=temp, source1=self.yt, source2=y_old, beta_1=1, beta_2=-1)
                    add_mome2(target=self.W, source1 = self.yt, source2 = temp, beta_1=1, beta_2 = gamma)

                elif self.args.method == 'ours':
                    for name in self.W:
                        self.W[name].data -= self.args.lr * self.mt[name].data.clone()


                elif (self.args.method == 'fedavg' or self.args.method == 'ours' or self.args.method == 'fedadam' or  self.args.method == 'fedavgm' or self.args.method == 'fedavg_gnag'
                      or self.args.method == 'fedprox'or self.args.method == 'fedacg' or self.args.method == 'fed_nesterov'
                      or self.args.method == 'fed_nesterov' or self.args.method == 'scaffold' or self.args.method == 'feddyn' or self.args.method == 'mime'):
                     optimizer.step()
...
class Server(DistributedTrainingDevice):
    ...
    def __init__(self, model, args):
        ...
        # Parameters
        self.W = {name: value for name, value in self.model.named_parameters()}
        self.pW = {name: value for name, value in self.model.named_parameters()}
        ...
        self.vt = {name: torch.zeros(value.shape).to(self.args.device) for name, value in self.W.items()}
        self.nt = {name: torch.zeros(value.shape).to(self.args.device) for name, value in self.W.items()}
        self.Wold = {name: torch.zeros(value.shape).to(self.args.device) for name, value in self.W.items()}
        ...
        if self.args.method == 'fed_nesterov' or self.args.method == 'ours':
        ...
        if self.args.method == 'fedacg':
            self.m_buff = {name: torch.zeros(value.shape).to(self.args.device) for name, value in self.W.items()}
        if self.args.method == 'fastslowmo':
            self.yg = {name: torch.zeros(value.shape).to(self.args.device) for name, value in self.W.items()}
            self.yl = {name: torch.zeros(value.shape).to(self.args.device) for name, value in self.W.items()}
            if iter == 0:
                self.yg = copy.deepcopy(self.model)
                self.yl = copy.deepcopy(self.model)
        self.local_epoch = 0
    ...
    def aggregate_weight_updates(self, clients, iter, aggregation="mean"):
        ...
        if self.args.method == 'fastslowmo':
            average(target=self.yl, sources=[client.yt for client in clients])
    ...
    def computer_weight_update_down_dw(self, clients, iter):
        ...
        if self.args.method == 'fedavgm':
            ...
            lambda_acg = self.args.beta_
            add_mome2(target=self.m_buff, source1 =self.m_buff, source2=self.dW, beta_1=lambda_acg, beta_2=1)
            add_mome2(target=self.W, source1 =self.W, source2=self.m_buff, beta_1=1, beta_2=1)
            ...
            gamma_g = 0.3
            temp = {name: torch.zeros(value.shape).to(self.args.device) for name, value in self.W.items()}
            scale_ts(target=temp, source= self.yg, scaling=1)
            add_mome2(target=self.yg, source1= self.W, source2=self.dW, beta_1=1, beta_2=1)
            add_mome2(target=temp, source1= self.yg, source2=temp, beta_1=1, beta_2=-1)
            add_mome2(target=self.W, source1=self.yg, source2=temp, beta_1=1, beta_2=gamma_g)
            ...
            add_mome2(target=self.vt, source1=self.vt, source2= dW_square, beta_1=self.args.beta_0, beta_2=1-self.args.beta_0)
        ...
        elif self.args.method == 'fedcm':
            scale_ts(target=self.dW, source=self.dW, scaling=(-1/(self.args.lr*self.local_epoch*self.args.local_ep)))
        ...
        if self.args.method == 'fed_localnesterov_nocom':
            ...
            add_mome2(target=self.v, source1=self.v, source2=self.dW, beta_1=self.args.beta_/(1+self.args.beta_),
                      beta_2= -1/(self.local_epoch * self.args.local_ep * (1+self.args.beta_)*self.args.lr))
